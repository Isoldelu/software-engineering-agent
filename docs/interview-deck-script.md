...
> 这是我基于公开风格模拟数据完成的个人复现项目，不包含华为内部数据。项目面向软件研发资产查询与分析，通过 Agent 组合结构化工具和 RAG，并把每次执行转化为可验证、可回放、可评测的 Trace。当前版本通过 134 项自动化测试和 193 条冻结评测，全程没有调用付费 API。
...
> 当前 GitHub Actions 同时验证了 134 项测试、Docker 构建和 PostgreSQL 多 Worker 路径。PostgreSQL 压测完成初始 100/100 和 Worker 替换后 40/40，6 项故障门禁通过。受控自进化不会自动修改源码或跳过人工审核。下一项独立实验是小预算真实 Provider A/B，需要显式 API Key、预算上限和成本记录，但它不阻塞当前项目作为完整 AI4SE Agent 的交付。
